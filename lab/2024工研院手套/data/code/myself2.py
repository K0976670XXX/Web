...
def load_images():
  ...
  images = {
    ...
    "left_cue_pic" : plt.imread('exp_img/left_cue_pic.jpg'),
    "right_cue_pic": plt.imread('exp_img/right_cue_pic.jpg')
  }
  return images
...
def BCI_KneeBO(event_stream_name='BCI - LSL Marker', kneebo_server_ip='127.0.0.6', kneebo_server_port=56950):
    ...
    if phase_select == 'Test':
      leg_name, leg_select, y_label = leg_selection()
      print('\n'+'#'*50)
...
class Test_function(object):
  ...
  def test_ExperimentFigures(self):
    ...
    cue_pic_dict.update({41: images['left_cue_pic']})
    cue_pic_dict.update({61: images['right_cue_pic']})
    task_order_list = generate_task_order(cue_pic_dict, 2)
    score = 0
  ...
  def test_KneeBO_control(self):
    ...
    while True:
      ...
      if udp_client_CMD == 'move' or udp_client_CMD == 'flush' or udp_client_CMD == 'end':
