...
def model_list(path):
  ...
  for sub_name, suffixes in groups.items():
    ...
    # 如果所有模式都匹配，組合法有效
    if len(left_files)==5:
      output.append(f"左手【{sub_name}】")
      output_dict[dict_count]={'hand_name':'l','sub_name':sub_name}
      dict_count+=1
    if len(right_files)==5:
      output.append(f"右手【{sub_name}】")
      output_dict[dict_count]={'hand_name':'r','sub_name':sub_name}
      dict_count+=1
  # 給使用者選擇
  while True:
    print('輸入欲使用模型:')
    for idx, group in enumerate(output):
      hand_name = output_dict[idx]['hand_name']
      sub_name = output_dict[idx]['sub_name']
      fp = f'{path}/{sub_name}-kneebo_fs_psd_{hand_name}.npy'
    ...
    try:
      ...
      print('\n'+'#'*50)
      continue
    hand_name = output_dict[model_sel]['hand_name']
    sub_name = output_dict[model_sel]['sub_name']
    return sub_name,hand_name
def load_model(sub_name,hand_name,train_path='EEG Data/model'):
  """載入訓練數據和模型文件"""
  X_fs_psd_path = f'{train_path}/{sub_name}-kneebo_fs_psd_{hand_name}.npy'
  clf_psd_path  = f'{train_path}/{sub_name}-kneebo_trained_clf_psd_{hand_name}.joblib'
  clf_pcc_path  = f'{train_path}/{sub_name}-kneebo_trained_clf_pcc_{hand_name}.joblib'
  clf_csp_path  = f'{train_path}/{sub_name}-kneebo_trained_clf_csp_{hand_name}.joblib'
  csp_path      = f'{train_path}/{sub_name}-kneebo_trained_csp_{hand_name}.joblib'
  try:
...
def BCI_KneeBO(event_stream_name='BCI - LSL Marker'):
    ...
    if phase_select == 'Test':
      ...
      sub_name,hand_name = model_list(path=model_path)
      hand_select, y_label = hand_selection(hand_name)
      print(hand_select, y_label)
      X_fs_psd, clf_psd, clf_pcc, csp, clf_csp = load_model(sub_name,hand_name,train_path=model_path)
      print('\n'+'#'*50)
    ...
    elif phase_select == 'Test' and hand_name == 'r':
      cue_pic_dict.update({51: images['rest_cue_pic']})
      cue_pic_dict.update({611: images['right_cue_pic1']})
      cue_pic_dict.update({621: images['right_cue_pic2']})
    elif phase_select == 'Test' and hand_name == 'l':
      cue_pic_dict.update({51: images['rest_cue_pic']})
      cue_pic_dict.update({411: images['left_cue_pic1']})
      cue_pic_dict.update({421: images['left_cue_pic2']})
    # 訓練/測試 階段的試驗次數
    ...
    for task_i,task_order in enumerate(task_order_list):
      if task_i!=0 and task_i%50==0:#每10組休息一次
        exp_fig.disp_rest()
    ...
    if phase_select == 'Test':
      ...
      np.save('EEG Data'+'/'+sub_name+'-kneebo_y_test_'+hand_name+'.npy', y_test)
      np.save('EEG Data'+'/'+sub_name+'-kneebo_y_predict_'+hand_name+'.npy', y_predict)
