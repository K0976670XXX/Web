...
def arm_selection():
  """選擇 左臂/右臂"""
  while True:
    arm_select = input('Please select the side of arm:\n  1) Left arm\n  2) Right arm\n')
    if arm_select == '1':
...
class MachineLearning(object):
  ...
  def set_exclusion_criteria(self,non_topic_marker):
    ...
    # np.save('X_epoch.npy', self.X_epoch)
  ...
  def print_mix_result(self,trials_num):
    ...
    Accuracy = accuracy_score(y_test, y_test_predict_final)
    # Precision = precision_score(y_test, y_test_predict_final, average="weighted")
    # Recall = recall_score(y_test, y_test_predict_final, average="weighted")
    # F1 = f1_score(y_test, y_test_predict_final, average="weighted")
    
    pos_label = 61
    Precision = precision_score(y_test, y_test_predict_final, pos_label=pos_label)
    Recall = recall_score(y_test, y_test_predict_final, pos_label=pos_label)
    F1 = f1_score(y_test, y_test_predict_final, pos_label=pos_label)
...
if __name__ == '__main__':
  ## 選擇 左臂/右臂
  leg_name,non_topic_marker = arm_selection()
  ## 輸入資料集
  try:
